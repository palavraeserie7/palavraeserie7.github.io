...
async function init() {
    try {
        ...
        console.log("Aviso: Rodando em modo limitado (sem banco de dados)");
        document.getElementById("user-email").innerText = "Modo Offline/Visitante";
    }
}

// FUNÇÃO DO MENU (ABAS) - Agora totalmente independente
function switchTab(view) {
    console.log("Mudando para:", view);
    ...
    // 3. Atualiza a cor no menu lateral
...
async function executeSearchWithMode(mode) {
    ...
    if (!query || query.length < 2) { 
        alert("Por favor, digite sua pergunta na barra de busca no topo."); 
        return; 
    }
